...
198. `server/internal/gitx/project_lock.go` `AcquireProjectLock`：获取可取消的跨进程项目 Git 排他锁。
199. `server/internal/leases/checkpoint.go` `Service.CreateCheckpointUnderProjectLock`：在既有项目锁内安全持久化检查点。
200. `server/internal/mr/review_worker.go` `NewReviewWorker`：创建可恢复的 MR 自动评审与合并器。
201. `server/internal/mr/review_worker.go` `ReviewWorker.Start`：启动 MR 自动评审与合并轮询。
202. `server/internal/mr/review_worker.go` `ReviewWorker.Close`：停止 MR 评审轮询并等待退出。
203. `server/internal/mr/review_worker.go` `ReviewWorker.Scan`：扫描并处理待评审或待合并 MR。
