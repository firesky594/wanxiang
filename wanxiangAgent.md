...
| ---- | --------------------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R001 | 后台顶部栏目调整为左右结构。                                    | 已完成 | 已完成 Element Plus 自适应左侧导航与可关闭标签工作区，修复管理员过期会话 401、SSE Cookie 鉴权及折叠图标居中；前后端测试、构建、PM2 与健康检查均通过。                      |
| R002 | 设置 Agents 列表，可在右侧查看现有 Agents 并修改对应配置。      | 已完成 | 人工修复                                                                                                                                                                   |
| R003 | 项目支持执行`bash start.sh` 一键运行。                        | 已完成 | 人工修复                                                                                                                                                                   |
| R004 | 在`wanxiangAgent.md` 中写明前后端技术栈及实际使用的具体插件。 | 已完成 | 在R001期间修复                                                                                                                                                             |
| R005 | 将任务调度台改为可拖动的全画布 Agent 视图。                     | 已完成 | 已完成全画布 Agent 图片节点、名称与红绿连接状态、拖拽持久化及自适应布局；临时测试 3/3、基线 32/32、构建、PM2 与健康检查均通过。                                            |
| R006 | 隐藏 Agents 导航，并由调度台 Agent 卡片展示配置及修复任务抽屉。 | 已完成 | 已隐藏 Agents 导航并保留直达兼容；Agent 卡片可打开脱敏配置、保存和探测，修复任务入口零宽抽屉并补齐加载错误反馈；专项测试、基线 32/32、构建、浏览器、PM2 与健康检查均通过。 |
| R007 | 总管持续巡检并管理 Agent，修复任务重复创建及项目无人执行。       | 已完成 | 已补强总管巡检、持久化状态与 Agent 管理边界；新任务采用会话级幂等与成功后自动关闭抽屉；Project 可继续规划、分配、工作区和执行链，并加入项目锁、SSE 状态同步及发布回滚保护。已完成前后端构建、后端重启及直连和 Nginx 健康检查。 |
