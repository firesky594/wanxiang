...
func Migrate(ctx context.Context, conn *sql.DB) error {
	stmts := []string{
		...
		`create table if not exists tasks (id integer primary key, project_id integer, title text not null, description text not null, status text not null, priority integer not null default 0, manager_summary text not null default '', planning_attempts integer not null default 0, planning_error_class text not null default '', planning_next_retry_at text, planning_started_at text, planning_condition_hash text not null default '', created_at text not null)`,
		...
		`create table if not exists mr_review_jobs (id integer primary key, mr_id integer not null unique, status text not null, review_attempts integer not null default 0, merge_attempts integer not null default 0, next_retry_at text, processing_started_at text, last_error text not null default '', blocked_code text not null default '', decision_json text not null default '', created_at text not null, updated_at text not null)`,
		`create index if not exists idx_mr_review_jobs_status_retry on mr_review_jobs(status,next_retry_at,processing_started_at)`,
		...
		`create table if not exists pipeline_rollbacks (id integer primary key, run_id integer not null unique, safe_commit text not null, expected_head text not null default '', status text not null, confirmed_by text not null default '', confirmed_at text, started_at text, created_at text not null, completed_at text, last_error text not null default '')`,
		...
		`create index if not exists idx_agent_match_task_step_latest on agent_match_decisions(task_id,step_id,id desc)`,
		`create index if not exists idx_agent_match_system_blocked_selected_latest on agent_match_decisions(task_id,step_id,id desc) where created_by='system' and status in ('blocked: missing_config','waiting: probe','blocked: missing_resources') and selected_agent is not null and selected_agent<>''`,
		...
		`create table if not exists executor_runs (id integer primary key, task_id integer not null, step_id integer not null, agent_name text not null, lease_id text not null unique, lease_version integer not null, pid integer, pid_start_ticks integer not null default 0, status text not null, claim_token text not null default '', claim_owner text not null default '', claim_expires_at text, launch_count integer not null default 0, continuation_count integer not null default 0, request_count integer not null default 0, input_tokens integer not null default 0, output_tokens integer not null default 0, exit_code integer, last_heartbeat_at text, error_summary text not null default '', created_at text not null, started_at text, exited_at text, updated_at text not null)`,
	...
	for _, column := range []struct {
		...
		{"tasks", "idempotency_key", "text not null default ''"},
		{"tasks", "request_fingerprint", "text not null default ''"},
		{"tasks", "planning_attempts", "integer not null default 0"},
		{"tasks", "planning_error_class", "text not null default ''"},
		{"tasks", "planning_next_retry_at", "text"},
		{"tasks", "planning_started_at", "text"},
		{"tasks", "planning_condition_hash", "text not null default ''"},
		...
		{"pipeline_rollbacks", "expected_head", "text not null default ''"},
		{"workflow_edges", "plan_version", "integer not null default 1"},
		{"executor_runs", "pid_start_ticks", "integer not null default 0"},
		{"executor_runs", "claim_token", "text not null default ''"},
		{"executor_runs", "claim_owner", "text not null default ''"},
		{"executor_runs", "claim_expires_at", "text"},
		{"executor_runs", "launch_count", "integer not null default 0"},
		{"executor_runs", "continuation_count", "integer not null default 0"},
	} {
	...
	if _, err := conn.ExecContext(ctx, `create unique index if not exists idx_tasks_idempotency_key on tasks(idempotency_key) where idempotency_key<>''`); err != nil {
		return err
	}
	if _, err := conn.ExecContext(ctx, `create index if not exists idx_tasks_planning_retry on tasks(status,planning_error_class,planning_next_retry_at)`); err != nil {
		return err
	}
	if _, err := conn.ExecContext(ctx, `create index if not exists idx_tasks_planning_started on tasks(status,planning_started_at)`); err != nil {
		return err
	}
	if _, err := conn.ExecContext(ctx, `create index if not exists idx_executor_runs_claim on executor_runs(claim_token,claim_expires_at)`); err != nil {
		return err
	}
	return nil
}
