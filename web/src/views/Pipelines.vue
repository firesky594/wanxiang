<template><section class="console"><main class="main"><div class="page-head"><div><p class="eyebrow">LOCAL PIPELINE</p><h1>测试、发布与回滚</h1><p>测试和构建可自动执行；发布、迁移、删除与回滚必须单独确认。</p></div><el-button :loading="loading" @click="load">刷新</el-button></div><el-alert title="高风险确认边界" description="验收不代表发布授权。确认前请核对安全提交、产物哈希和回滚入口。" type="warning" :closable="false"/><section class="runs"><article v-for="run in runs" :key="run.ID" class="panel"><header><strong>RUN-{{run.ID}} · PROJECT-{{run.ProjectID}}</strong><el-tag>{{run.Status}}</el-tag></header><p class="mono">SAFE {{run.SafeCommit||'—'}} · ARTIFACT {{run.ArtifactHash||'—'}}</p><el-button v-if="run.RollbackStatus==='awaiting_confirmation'" type="danger" @click="rollback(run.ID)">确认回滚到安全提交</el-button><div v-for="step in run.Steps" :key="step.ID" class="step"><span>{{step.Kind}} / {{step.Key}}</span><b>{{step.Status}}</b><small>尝试 {{step.Attempt}} / {{step.MaxAttempts}} · {{step.FailureClass}}</small><code>{{step.Command}} {{step.Args.join(' ')}}</code><el-button v-if="step.Status==='awaiting_confirmation'" type="danger" plain @click="confirm(run.ID,step.Key)">确认高风险步骤</el-button></div></article><div v-if="!runs.length" class="panel empty">暂无流水线运行</div></section></main></section></template>
