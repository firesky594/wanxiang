...
func Migrate(ctx context.Context, conn *sql.DB) error {
	stmts := []string{
		...
		`create table if not exists pipeline_runs (id integer primary key, project_id integer not null, task_id integer, definition_json text not null, definition_hash text not null, status text not null, safe_commit text not null default '', artifact_hash text not null default '', idempotency_key text not null unique, requested_by text not null, created_at text not null, started_at text, completed_at text, last_error text not null default '')`,
		`create table if not exists pipeline_steps (id integer primary key, run_id integer not null, step_key text not null, kind text not null, command text not null, args_json text not null, artifact text not null default '', timeout_seconds integer not null, max_attempts integer not null, reversible integer not null default 1, status text not null, attempt integer not null default 0, failure_class text not null default '', output_summary text not null default '', next_retry_at text, confirmed_by text not null default '', confirmed_at text, started_at text, completed_at text, unique(run_id,step_key))`,
		`create index if not exists idx_pipeline_steps_status on pipeline_steps(status,next_retry_at)`,
		`create table if not exists pipeline_rollbacks (id integer primary key, run_id integer not null unique, safe_commit text not null, status text not null, confirmed_by text not null default '', confirmed_at text, created_at text not null, completed_at text, last_error text not null default '')`,
